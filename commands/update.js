...
function downloadFile(url, dest, visited = new Set()) {
    return new Promise((resolve, reject) => {
        try {
            ...
            const req = client.get(url, { headers: { 'User-Agent': 'BLACK-HATBOT-Updater/1.0', 'Accept': '*/*' } }, res => {
...
async function updateViaZip(sock, chatId, message, zipOverride) {
    ...
    // Preserve owner/botOwner settings
    let preservedOwner = settings.ownerNumber ? String(settings.ownerNumber) : null;
    let preservedBotOwner = settings.botOwner ? String(settings.botOwner) : null;
